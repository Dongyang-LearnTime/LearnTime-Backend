...
public class MyPageController {
    ...
    public ResponseEntity<MyPageResponseDTO> getMyPage(@AuthenticationPrincipal CustomUserDetails user) {
        MyPageResponseDTO result = myPageService.getMyInfo(user.getUsername());
    ...
    public ResponseEntity<MyPageSummaryResponseDTO> getMySummary(@AuthenticationPrincipal CustomUserDetails user) {
        MyPageSummaryResponseDTO result = myPageService.getMySummary(user.getUsername());
        return ResponseEntity.ok(result);
    }
    ...
    public ResponseEntity<PageResponse<PostListResponseDTO>> getMyPosts(
        ...
        PageResponse<PostListResponseDTO> result = myPageService.getMyPosts(user.getUsername(), pageable);
        return ResponseEntity.ok(result);
    }
    ...
    public ResponseEntity<PageResponse<MyCommentListResponseDTO>> getMyComments(
            @AuthenticationPrincipal CustomUserDetails user,
            @PageableDefault(size = 10, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable) {
        PageResponse<MyCommentListResponseDTO> result = myPageService.getMyComments(user.getUsername(), pageable);
        return ResponseEntity.ok(result);
    }
