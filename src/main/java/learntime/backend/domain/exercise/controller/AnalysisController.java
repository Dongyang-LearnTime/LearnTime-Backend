...
@RestController
@RequestMapping("/api/user/analysis") // 인증된 사용자 전용 경로
...
public class AnalysisController {
    ...
    @GetMapping("/weekly")
    public ResponseEntity<AnalysisResponseDTO> getWeeklyAnalysis(
            @AuthenticationPrincipal User user) {

        // 7일 분석 서비스 호출
        AnalysisResponseDTO response = analysisService.getWeeklyAnalysis(user);
