...
import learntime.backend.domain.study_progress.dto.StudyDailyPlanStatsDTO;
...
public interface StudyStatusRepository extends JpaRepository<StudyStatus, Long> {
    ...
    // 완료된 일일 계획의 총 개수를 조회함
    @Query("SELECT COUNT(s) FROM StudyStatus s WHERE s.studyMember.studyMemberId = :studyMemberId AND s.progressStatus = 'COMPLETED'")
    long countCompletedPlansByStudyMemberId(@Param("studyMemberId") Long studyMemberId);

    // 멤버의 통계 지표를 위한 상태를 조회함
    @Query("SELECT new learntime.backend.domain.study_progress.dto.StudyDailyPlanStatsDTO(s.progressStatus, s.completionStatus, s.focusTime) " +
