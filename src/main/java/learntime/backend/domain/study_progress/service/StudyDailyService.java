...
public class StudyDailyService {
    ...
    public int completeStudyDailyPlan(PlanCompleteRequestDTO request, Long userId) {
        ...
        // 진도 전체 완료(개인) 검증
        long totalPlans = studyDailyPlanRepository.countByStudy_StudyId(studyDailyPlan.getStudy().getStudyId());
        long completedPlans = studyStatusRepository.countCompletedPlansByStudyMemberId(studyMember.getStudyMemberId());

        if (totalPlans > 0 && totalPlans == completedPlans) {
            studyMember.complete();
            studyMemberRepository.save(studyMember);
            
            eventPublisher.publishEvent(new PointEventDTO(
                    userId,
                    PointPolicy.STUDY_TOTAL_COMPLETED.getAmount(),
                    PointType.EARN,
                    PointPolicy.STUDY_TOTAL_COMPLETED.getDescription()
            ));
        }
    ...
    public void finalizeExpiredStudyMembers() {
        ...
        int count = 0;
        for (StudyMember member : expiredMembers) {
            member.complete();
        ...
        log.info("[StudyDailyPlan] 종료된 스터디 멤버 완료 처리 완료 - 대상자 수: {}명", count);
    }
